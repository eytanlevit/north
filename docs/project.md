...
The agent runs as a `claude -p --output-format stream-json` subprocess (headless print mode with structured JSON output). North parses the JSON-lines stream and renders messages, tool calls, and results in the chat pane. The kanban board watches `.north/` via fsnotify and updates live as the agent creates/updates issues.
...
### Skill: `north` (planned for M4)
A Claude Code skill (installed at `~/.claude/skills/north.md` or bundled) that will provide the same guidance for agents working outside the repo context. Not yet built — currently, agents get context via the `.north/CLAUDE.md` file generated by `north init`.
...
### M3: Session Integration (tmux prototype — deprecated)
...
### M3: Integrated Agent TUI
- `north session` — single Bubble Tea app with agent chat + kanban board
- Spawn `claude -p --output-format stream-json` subprocess (headless print mode)
...
| Session | Embedded agent (subprocess) | Single binary, unified UX, no external dependencies |
