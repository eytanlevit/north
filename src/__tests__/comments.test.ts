...
describe("comments", () => {
  ...
  it("adds a comment to an issue", () => {
    ...
    const updated = addComment(tmpDir, "NOR-001", comment);
  ...
  it("adds multiple comments and preserves order", () => {
    ...
    addComment(tmpDir, "NOR-001", c1);
    const updated = addComment(tmpDir, "NOR-001", c2);
  ...
  it("comments survive round-trip (write + read)", () => {
    ...
    writeIssue(tmpDir, makeIssue());
    addComment(tmpDir, "NOR-001", comment);
  ...
  it("throws when adding comment to non-existent issue", () => {
    ...
    expect(() => addComment(tmpDir, "NOR-999", comment)).toThrow("Issue NOR-999 not found");
  });
});
