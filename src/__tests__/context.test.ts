...
function makeTmpDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "pmtui-test-"));
}
...
function writeDoc(cwd: string, relPath: string, content: string): void {
  const fullPath = path.join(cwd, ".pm", relPath);
...
describe("buildContext", () => {
  ...
  it("returns basic issue context with no blockers or docs", () => {
    ...
    const result = buildContext({ cwd, issueId: "ISS-001" });

    expect(result.issue.id).toBe("ISS-001");
  ...
  it("throws when issue does not exist", () => {
    expect(() => buildContext({ cwd, issueId: "ISS-999" })).toThrow(
      "Issue ISS-999 not found"
    );
  });
  ...
  it("includes blocking issue details", () => {
    ...
    const issue = makeIssue({ blocked_by: ["ISS-002"] });
    writeIssue(cwd, issue);
  ...
  it("handles missing blocked_by references gracefully", () => {
    const issue = makeIssue({ blocked_by: ["ISS-999"] });
    writeIssue(cwd, issue);
  ...
  it("includes parent issue when present", () => {
    ...
    const issue = makeIssue({ parent: "ISS-010" });
    writeIssue(cwd, issue);
  ...
  it("handles missing parent gracefully", () => {
    ...
    if ("error" in result.parentIssue!) {
      expect(result.parentIssue.id).toBe("ISS-999");
  ...
  it("handles issue with comments", () => {
    const issue = makeIssue({
      comments: [
        { author: "alice", date: "2026-01-02", text: "Looks good" },
        { author: "bob", date: "2026-01-03", text: "Agreed" },
      ],
    });
    writeIssue(cwd, issue);
    ...
    expect(result.issue.comments![1].text).toBe("Agreed");
  });
});
...
describe("formatContext", () => {
  it("produces expected text sections for a basic issue", () => {
    ...
    expect(text).toContain("# Issue: ISS-001");
  ...
  it("includes all sections when present", () => {
    const result: ContextResult = {
      ...
      issue: makeIssue({
        comments: [{ author: "alice", date: "2026-01-02", text: "A comment" }],
      }),
      parentIssue: makeIssue({ id: "ISS-010", title: "Parent" }),
      blockingIssues: [
        makeIssue({ id: "ISS-002", title: "Blocker", status: "in-progress" }),
        { id: "ISS-003", error: "not found" },
      ],
      linkedDocs: [
...
describe("formatContextJson", () => {
  it("produces valid JSON", () => {
    const result: ContextResult = {
      project: "My project",
      issue: makeIssue(),
      blockingIssues: [{ id: "ISS-002", error: "not found" }],
    ...
    expect(parsed.issue.id).toBe("ISS-001");
    expect(parsed.blockingIssues[0].id).toBe("ISS-002");
